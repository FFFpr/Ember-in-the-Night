...
| 判据 | 要求 | 怎么量 |
|------|------|--------|
| 暗部收边 | 边缘均值 ≤ 全幅 P90 × **0.40** | 四边各 10% 宽条带的均值，比全幅灰度的第 90 百分位 |
...
暗部收边不要拿「边缘 / 中心」来量：主体不在画面正中时这个比值会误判。用全幅 P90 代表主体亮度，与构图无关。为了让文字可读而补的光要**局部**（贴着文字或看板的小范围光），否则整体亮度和收边两条会一起失败。
